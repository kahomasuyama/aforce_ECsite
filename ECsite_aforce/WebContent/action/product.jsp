...
<body>
	...
	<div class="search">
		...
		<form action="ProductList.action" method="post">
			...
			<input type="submit" value="検索" class="button searchBtn">
		</form>
		<br>
		<p>価格帯検索</p>
	...
	<table style="border-spacing: 10px; display: table; width: 100%; border-collapse: separate; text-align: center;">
		<c:forEach var="product" items="${list}">
			<tr style="display: inline-block; width: 30%; text-align: center;" class="list table">
				<td class="imgWrap">
					<img src="image/${product.id}.jpg" height="250" width="90%">
				</td>
				<td>${product.name}</td>
				<td>￥${product.price}/個</td>
				<td>
					<p>購入個数：</p>
					<form action="CartAdd.action?id=${product.id}" method="POST">
						...
						<input type="submit" value="カートに追加" class="button searchBtn">
					</form>
				</td>
			</tr>
		</c:forEach>
